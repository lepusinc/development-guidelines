...
5. **コミット運用（参照）**  
   - コミットメッセージ規約は [Gitワークフローガイドライン](GIT_WORKFLOW.md#6-コミットメッセージ) を参照。  
...
レビューコメント（主に CodeRabbit）は、CodeRabbit の **severity** ラベルでトリアージする。この軸は個々の指摘の緊急度を表し、以下の2つの軸とは別物である。

- 上記の **レビューレベル（L0/L1/L2）** — *PR 全体* のリスクを分類する。
- **ガイドラインの severity ラベル**（`[厳守]` / `[要修正]` / `[条件付き]` / `[推奨]`、定義は [AGENTS.md](AGENTS.md)）— *ガイドラインのルール* をどれだけ厳守すべきかを分類する。

CodeRabbit の severity に応じて、以下のとおり対応する。

| CodeRabbit Severity | この PR での対応 | フォローアップ課題 | ガイドラインラベル（≈） |
| --- | --- | --- | --- |
| Critical | 必ず修正（ブロッカー） | 不可 | `[厳守]` |
| Major | 原則修正。遅延はチケット起票が必須 | 可（チケット起票が必須） | `[要修正]` |
| Minor | 即修正、またはフォローアップ課題を作成 | 可 | `[条件付き]` / `[推奨]` |
| Trivial / Nitpick | 任意 | 可（または won't-fix） | `[推奨]` |

Minor 以下のコメントをフォローアップ課題で後送りする手順（必須の課題リンクを含む）は、Dev Playbook「Jira 課題運用ガイド」（§7 フォローアップ課題）を参照: <https://lepus.atlassian.net/wiki/spaces/DEVPB/pages/41615364/Jira>。
